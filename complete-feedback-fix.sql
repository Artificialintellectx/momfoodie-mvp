-- Complete Fix for User Feedback Table
-- This script handles all the issues: foreign key constraints, type mismatches, and missing columns
...
-- Show the meals table structure to understand what columns exist
SELECT column_name, data_type, is_nullable, column_default 
FROM information_schema.columns 
...
-- Step 11: Recreate the meal_stats view with only existing columns and proper type casting
...
LEFT JOIN (
    ...
    GROUP BY meal_id
) feedback_stats ON m.id::VARCHAR(255) = feedback_stats.meal_id
ORDER BY m.name;
