-- Fix User Feedback Table Schema
-- This script adds missing columns to the existing user_feedback table
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE constraint_name LIKE '%user_feedback_rating_check%'
    ) THEN
        ALTER TABLE user_feedback ADD CONSTRAINT user_feedback_rating_check CHECK (rating >= 1 AND rating <= 5);
        RAISE NOTICE 'Added rating constraint';
    END IF;
